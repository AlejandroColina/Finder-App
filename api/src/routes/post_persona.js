...
router.post('/', async (req, res, next) => {
    try {
        ...
        if (consultaBD.length) {
            ...
            if (!validateEmail.test(email.replaceAll('"', ''))) return res.status(404).send('ERROR CON EL CORREO ELECTRONICO');
            if (
                typeof edad !== 'number'
            ...
            if (nombres === undefined
                ...
                return res.status(404).send('FALTAN DATOS REQUERIDOS PARA CREAR USUARIO.');
            }
            else {
                ...
                await Direccion.create({ direccion, PersonaId, pais: 'COL', ciudad: 'PER' });
